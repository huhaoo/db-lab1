...
public:
	...
	std::optional<std::string_view> FindSlot(std::string_view key) const {
		auto id=Find(key);
		if(id==SlotNum()) return std::nullopt;
		return Slot(id);
		// DEBUG
	}
	...
	void AppendSlotUnchecked(std::string &slot){ AppendSlotUnchecked(std::string_view(slot.begin(),slot.end())); }
	...
	bool SplitInsert(SortedPage<SlotKeyCompare, SlotCompare>& right,
		...
		std::vector<std::string> slots;
		if(SlotNum()==slotid) slots.push_back(std::string(slot));
		while(SlotNum()){ slots.push_back(std::string(Slot(SlotNum()-1))); SlotNumMut()--; if(SlotNum()==slotid) slots.push_back(std::string(slot)); }
		std::vector<uint32_t> len(slots.size());
		for(size_t i=0;i<slots.size();i++) len[i]=sizeof(pgoff_t)+slots[i].size();
		for(size_t i=1;i<slots.size();i++) len[i]+=len[i-1];
		size_t p=0;
		while(len[p]*2<=len.back()) p++;

		Assert(p&&p!=len.size());
		while(slots.size()>p){ Assert(IsInsertable(slots.back())); AppendSlotUnchecked(slots.back()); slots.pop_back(); }
		while(!slots.empty()){ Assert(right.IsInsertable(slots.back())); right.AppendSlotUnchecked(slots.back()); slots.pop_back(); }
